...
@WebServlet("/member/freeboard/reg")

@MultipartConfig(location = "C:\\workspace\\JCCproject\\WebContent\\upload", fileSizeThreshold = 1024 * 1024, maxFileSize = 1024 * 1024 * 5, // 5占쌨곤옙
maxRequestSize = 1024 * 1024 * 5 * 5 // 5占쌨곤옙 5占쏙옙占쏙옙占쏙옙
		)
...
public class RegController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1.占쏙옙占싸듸옙 占쏙옙罐占� 占쏙옙占�
		String urlPath = "/upload";
		String path = request.getServletContext().getRealPath(urlPath);

		//2. 占쏙옙占싸듸옙占� 占쏙옙占싹몌옙 占쏙옙占퐏
		String fileName = filePart.getSubmittedFileName();

		//3. 占쏙옙慣占쏙옙占쏙옙占�
		String filePath = path+File.separator+fileName; //File.separator-> "\\"

		//4. 占쏙옙寬占� 占쏙옙占쌕댐옙 占쏙옙占쏙옙 占쏙옙占쏙옙
		File pathFile = new File(path);
		if(!pathFile.exists())//占쏙옙占쏙옙占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙
			pathFile.mkdirs();//占쏙옙占쏙옙占쏙옙占쌍쇽옙占쏙옙
